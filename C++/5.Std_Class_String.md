...
**1. `char str[] = "牛马";` —— GBK 也就是“土著”**

```C++
char str[] = "牛马"; 
// cout << sizeof(str) << endl; // 输出 5
```

- **原理**：
	- 这里的 `char` 是传统的字节桶。因为环境是 Windows 且未强制指定 UTF-8，编译器默认使用 **GBK** 编码。
	- **GBK 规则**：一个汉字 = 2 个字节。
	- **计算**：2个字 × 2字节/字 + 1个 `\0` = **5 字节**。
- **看调试器数值 (`str`)**：
	- `str[0] = -59`  (`0xC5`)、`str[1] = -93`  (`0xA3`)
	- **解释**：`0xC5A3` 正是“牛”字的 GBK 编码！为什么是负数？因为 `char` 默认是有符号的 (`signed char`)，`0xC5` (197) 大于 127，被当成了负数显示。
	- `str[2] = -62` (`0xC2`)、`str[3] = -19` (`0xED`)
	- **解释**：`0xC2ED` 正是“马”字的 GBK 编码。

**2. `char16_t str16[] = u"牛马";` —— UTF-16 (定长小弟)**

```C++
char16_t str16[] = u"牛马";
// cout << sizeof(str16) << endl; // 输出 6
```

- **原理**：
	- 前缀 **`u`** 强制告诉编译器：“不管源文件是什么编码，请把这两个字转换成 **UTF-16** 存进去”。
	- **UTF-16 规则**：常用汉字 = 2 个字节 (`char16_t`)。
	- **计算**：(2个字 + 1个 `\0`) × 2字节/元素 = **6 字节**。
- **看调试器数值 (`str16`)**：
	- `str16[0] = 29275`
	- **解释**：29275 换算成十六进制是 **`0x725B`**。去查 Unicode 表，`U+725B` 正是“牛”。
	- `str16[1] = 39532`
	- **解释**：39532 换算成十六进制是 **`0x9A6C`**。`U+9A6C` 正是“马”。
	- **注意**：这里存的是**Unicode 码点**（在基本多文种平面内，UTF-16 的值等于码点）。

**3. `char32_t str32[] = U"牛马";` —— UTF-32 (定长土豪)**

```C++
char32_t str32[] = U"牛马";
// cout << sizeof(str32) << endl; // 输出 12
```

- **原理**：
	- 前缀 **`U`** (大写) 强制使用 **UTF-32**。
	- **UTF-32 规则**：不管什么字，统统占 4 个字节 (`char32_t`)。
	- **计算**：(2个字 + 1个 `\0`) × 4字节/元素 = **12 字节**。
- **看调试器数值 (`str32`)**：
	- 数值和 `str16` 一模一样（29275 和 39532）。
	- **区别**：虽然数值一样，但它们在内存里占的坑位不同。`str16` 占 2 字节，`str32` 占 4 字节（高位补0）。

**4. `wchar_t wstr[] = L"牛马";` —— 随波逐流的变色龙**

```C++
wchar_t wstr[] = L"牛马";
// cout << sizeof(wstr) << endl; // 6
```

- **原理**：
	- 前缀 **`L`** 表示宽字符。
	- **重点**：`wchar_t` 的大小取决于操作系统！
	- **Windows 上**：`wchar_t` = 2 字节 (UTF-16)。所以它的表现和 `char16_t` **完全一致**。
	- **Linux 上**：`wchar_t` = 4 字节 (UTF-32)。如果你把这段代码放到 Linux 跑，输出就会是 12。
- **计算 (Windows)**：(2个字 + 1个 `\0`) × 2字节/元素 = **6 字节**。

![image-20260207132812105](./5.Std_Class_String.assets/image-20260207132812105.png)

**总结对照表**

| **变量** | **前缀** | **C++ 类型** | **对应编码**     | **一个字占几字节** | **字符串总字节数 (含\0)** | **调试器里的值 (十进制)** |
| -------- | -------- | ------------ | ---------------- | ------------------ | ------------------------- | ------------------------- |
| `str`    | 无       | `char`       | **GBK**          | 2                  | 5                         | -59 (0xC5)...             |
| `str16`  | `u`      | `char16_t`   | **UTF-16**       | 2                  | 6                         | 29275 (0x725B)            |
| `wstr`   | `L`      | `wchar_t`    | **UTF-16** (Win) | 2                  | 6                         | 29275 (0x725B)            |
| `str32`  | `U`      | `char32_t`   | **UTF-32**       | 4                  | 12                        | 29275 (0x725B)            |

**核心结论**

1. **GBK 是“负数”的根源**：看到 `char` 数组里全是负数，且两个一组，基本就是 GBK 无疑。
2. **Unicode 是统一的 ID**：你看 `str16`, `str32`, `wstr` 里的数值（29275, 39532）都是一样的，因为它们都指向同一个 Unicode 码点。区别只在于**用多大的盒子去装这个数字**。
3. **Visual Studio 的默认行为**：如果你不加特殊设置，VS 默认把源码里的中文按 GBK 处理给 `char`。这就是为什么 `sizeof(str)` 是 5 而不是 7 (UTF-8) 的原因。
...
这种写法被称为 **“现代版写法” (Modern C++ Idiom)**，或者更学术的叫法是 **Copy-and-Swap Idiom (拷贝-交换惯用法)**。

它的核心思想是：**复用**（剥削编译器自动生成的代码）和 **RAII**（资源获取即初始化，让局部对象自动管理生命周期）。

这种写法相对于传统的“手动开空间、拷贝、释放旧空间”的写法，代码更简洁，且具有**强异常安全性**。 

**这种写法的三大好处**

**1. 代码复用极高 (DRY 原则)**

- 你不需要在拷贝构造函数里写一遍 `new`，也不需要在赋值重载里写一遍 `new` 和 `delete`。
- 所有的资源申请都集中在构造函数，所有的释放都集中在析构函数。赋值操作只是把它们串起来。

**2.强异常安全性 (Strong Exception Safety)**

这是高级 C++ 面试的杀手锏。

- **传统写法**：先 `delete` 旧空间，再 `new` 新空间。如果 `new` 失败抛出异常，旧空间已经释放了，对象变成“半死不活”的状态（野指针），数据也丢了。
- **现代写法**：
	- 在进入 `operator=` 函数体**之前**（传参构造 `tmp` 时），内存分配就已经发生了。
	- 如果 `new` 失败，异常会在构造 `tmp` 时抛出，此时还没进入函数，`s1` 还没被修改。
	- 只要进入了函数体，`swap` 操作是绝不会抛异常的。
	- **结论**：要么赋值完全成功，要么保持原样，绝不会破坏原对象数据。

**3. 极简逻辑**

- **自赋值检查**：传统写法必须写 `if (this != &s)`。现代写法不需要！如果是 `s1 = s1`，`tmp` 拷贝了一份 `s1`，`s1` 和 `tmp` 交换，`tmp`（拿着旧 `s1`）销毁。虽然效率稍微低一点点（多了一次拷贝），但逻辑完全正确，代码更少。
- **内存泄漏**：无需手动 `delete`，利用局部对象 `tmp` 的析构函数自动清理，杜绝内存泄漏 。

## 1.13 引用计数和写时拷贝

### 1. 引用计数 (Reference Counting)

引用计数是用来记录**“有多少个对象正在共享同一块内存资源”**的技术 。

- **比喻**：以前是大家看电视，每个人买一台电视（深拷贝）。现在是大家共用一台电视，并在电视上贴个便签纸写上“正在看的人数：3”。
- **规则**：
	- **构造时**：贴个条，写“1”。
	- **拷贝时**（`s2 = s1`）：不买新电视，只是把条上的数字改成“2”。
	- **销毁时**（`~s1`）：把条上的数字减 1。
	- **释放时**：只有当条上的数字变成“0”时，才真正把电视扔掉（释放内存）。

### 2. 写时拷贝 (Copy-On-Write)

写时拷贝是在**浅拷贝**（共享内存）的基础上，为了解决**“修改冲突”**而引入的机制。它是一种“拖延症”策略 。

- **问题**：既然 `s1` 和 `s2` 共用一块内存，如果我修改 `s2`（比如 `s2[0] = 'x'`），`s1` 也会变，这不就乱套了吗？
- **解决**：在**读取**时，大家尽情共享。只有当某个人试图**写入/修改**内容时，系统才会惊醒：“哎呀，不能影响别人”，于是赶紧为这个修改者**单独拷贝**一份数据，让他去改副本。

以下是在两种编译环境下对`string`代码进行测试得到的结果：

![image-20260211214025669](./5.Std_Class_String.assets/image-20260211214025669.png)

可以看到`linux`环境下就是按照上述思想进行实现的，但`VS`并不是，而且在`linux`测试时就发现，在`g++`编译环境下，`string`仅仅**只占8个字节**，这是因为在 G++（Linux 环境）的经典实现中，`string` 的结构非常精简，只包含一个指针，指向堆空间。堆空间里不仅存了字符串，还存了管理信息 。而`Linux`默认是64位。

![image-20260211214442760](./5.Std_Class_String.assets/image-20260211214442760.png)

### 3. 缺点

1. **线程安全问题**：引用计数必须是原子操作（Atomic），在多线程环境下，频繁的原子加减锁竞争会导致性能下降 。
2. **隐式开销**：每次通过 `[]` 操作符访问字符时，甚至只是读，`string` 都很难判断你是要读还是写（`operator[]` 返回的是引用），因此可能导致不必要的拷贝或复杂的检查逻辑。
3. **迭代器失效**：COW 机制可能导致迭代器在不知情的情况下失效（因为数据悄悄搬家了）。

### 4. 现状（特别注意）

虽然这是个非常经典的设计，但在 **C++11 标准之后，写时拷贝（COW）在 `std::string` 中实际上被“判了死刑”**。

- **原因**：C++11 规定了 `operator[]` 和迭代器的一些合法性保证，以及对多线程安全的高要求，COW 的实现变得极其复杂且难以满足标准。
- **现在的主流实现**：
	- **Visual Studio (MSVC)** 和 **现代 GCC (5.0+)** 都放弃了 COW。
	- 现在的 `string` 通常使用 **SSO (Small String Optimization)**：
		- **短字符串**：直接存在栈上（对象内部），不申请堆内存（类似于 `char buf[16]`）。
		- **长字符串**：老老实实申请堆内存，进行深拷贝。
