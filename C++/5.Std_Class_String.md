...
这是一个非常严谨的要求。为了展示**完全不简化**的接口原型，我们需要使用 C++ 标准文档中定义的类型名称（如 `size_type`、`const_iterator` 等），而不是简单的 `int` 或 `iter`。

以下是基于 C++11/C++20 标准的 `std::string` (即 `std::basic_string<char>`) 的完整接口原型。

### 术语对照表 (Legend)

在阅读以下原型前，请先了解这些标准类型的实际含义：

- `string&`: 即 `std::basic_string<char>&`，返回字符串自身的引用。
- `size_type`: 通常是 `size_t` (无符号整数)。
- `iterator`: 通常是 `char*` 或包装指针的类。
- `const_iterator`: 指向 `const char` 的迭代器。
- `npos`: 即 `std::string::npos` (通常为 -1 的无符号值)。

------

### 1. `insert` 完整接口原型总表

`insert` 分为两大类：返回**字符串引用**的（基于下标）和 返回**迭代器**的（基于迭代器）。

| **类型**   | **完整接口原型 (C++ Standard Signature)**                    | **参数解释**                                                 |
| ---------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **下标**   | `string& insert(size_type pos, const string& str);`          | 在下标 `pos` 插入整个对象 `str`                              |
| **下标**   | `string& insert(size_type pos, const string& str, size_type subpos, size_type sublen = npos);` | 在下标 `pos` 插入 `str` 中从 `subpos` 开始长度为 `sublen` 的部分 |
| **下标**   | `string& insert(size_type pos, const char* s);`              | 在下标 `pos` 插入 C 风格字符串 `s`                           |
| **下标**   | `string& insert(size_type pos, const char* s, size_type n);` | 在下标 `pos` 插入 `s` 的前 `n` 个字符                        |
| **下标**   | `string& insert(size_type pos, size_type n, char c);`        | 在下标 `pos` 插入 `n` 个字符 `c`                             |
| **迭代器** | `iterator insert(const_iterator pos, char c);`               | 在迭代器 `pos` 之前插入字符 `c`                              |
| **迭代器** | `iterator insert(const_iterator pos, size_type n, char c);`  | 在迭代器 `pos` 之前插入 `n` 个字符 `c`                       |
| **迭代器** | `template< class InputIt >` `iterator insert(const_iterator pos, InputIt first, InputIt last);` | 在迭代器 `pos` 之前插入 `[first, last)` 区间的内容           |
| **迭代器** | `iterator insert(const_iterator pos, std::initializer_list<char> ilist);` | (C++11) 在迭代器 `pos` 之前插入初始化列表 `{...}`            |

------

### 2. `erase` 完整接口原型总表

`erase` 同样分为返回**字符串引用**和返回**迭代器**两种。

| **类型**   | **完整接口原型 (C++ Standard Signature)**                    | **参数解释**                                                 |
| ---------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **下标**   | `string& erase(size_type pos = 0, size_type len = npos);`    | **最常用**。默认从 0 开始，默认删到末尾。返回修改后的字符串引用。 |
| **迭代器** | `iterator erase(const_iterator pos);`                        | 删除迭代器 `pos` 指向的字符。返回指向被删元素**后一个**位置的迭代器。 |
| **迭代器** | `iterator erase(const_iterator first, const_iterator last);` | 删除迭代器区间 `[first, last)` 的所有字符。返回指向最后被删元素**后一个**位置的迭代器。 |
